...
export default function BorrowerStepper({ onBack, initialLoanType }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const apiLenders = (lendersRes.data?.success && Array.isArray(lendersRes.data.data))
  ...
  const handleVerifyOtpAndSubmit = async () => {
    ...
    try {
      ...
      // 2. Register/Login borrower and submit applications
  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', width: '100%', animation: 'fadeUp 0.35s ease' }}>
      ...
      <div className="form-card">
        {/* ═══ STEP PROGRESS BAR (LOAN TYPE -> LENDERS -> LOGIN -> DONE) ═══ */}
        ...
        {loading ? (
          ...
          <>
              ...
              <div>
                ...
                <div className="bl-type-grid">
                  {loanTypesData.map((lt) => (
                    <div
                      key={lt.id}
                      className={`bl-type-card ${loanType === lt.id ? 'sel' : ''}`}
                      onClick={() => handleLoanTypeSelect(lt.id)}
                    >
            ...
            {/* ═══ STEP 3: LOGIN / REGISTER TO APPLY ═══ */}
            {step === 3 && (
            ...
            {/* ═══ STEP 4: DONE (REDIRECTS TO BORROWER DASHBOARD) ═══ */}
            {step === 4 && (
